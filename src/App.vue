<template>
  <div class="bg-dark h-screen">
    <div class="flex" style="height: 92vh;">
      <!-- side nav -->
      <aside class="w-56 bg-black h-full flex-none">
        <div class="p-6">
        ...
        <div class="mx-2">
          <button v-for="(page, index) in pages" :key="index" @click="selectedID = page.id" :class="`w-full rounded px-3 py-2 text-sm font-semibold text-left flex items-center justify-start ${selectedID == page.id ? 'bg-light text-white' : 'text-lightest'}`">
            <i class="material-icons md-light mr-3">{{ page.icon }}</i> 
            <span>{{ page.name }}</span>
          </button>
        </div>
      </aside>
